...
export default defineComponent({
  ...
  setup() {
    ...
    const lrcTop = ref("80px"); // Lyrics slide
    const lyricArr = ref([]); // The lyrics of the current song
    const songId = computed(() => store.getters.songId); // Song ID
    const lyric = computed(() => store.getters.lyric); // lyric
    const currentPlayList = computed(() => store.getters.currentPlayList); // Stored music
    const currentPlayIndex = computed(() => store.getters.currentPlayIndex); // The location of the current song in the song list
    const curTime = computed(() => store.getters.curTime);
    const songTitle = computed(() => store.getters.songTitle); // Song name
    const singerName = computed(() => store.getters.singerName); // Singer's name
    const songPic = computed(() => store.getters.songPic); // Song picture
    watch(songId, () => {
